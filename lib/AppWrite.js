...
export const saveVideo = async (data) => {
  try {
    // Fetch the current document to get the existing 'users' array
    const videoDoc = await databases.getDocument(
      databaseId,         // Your Appwrite database ID
      videoCollectionId,   // Your Appwrite video collection ID
      data.video           // Document ID (video in this case)
    );

    // Ensure users is an array and append the new user ID
    const currentUsers = Array.isArray(videoDoc.users) ? videoDoc.users : [];
    
    // Add new user ID if it's not already in the array
    if (!currentUsers.includes(data.user)) {
